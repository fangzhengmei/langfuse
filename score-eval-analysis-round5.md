...
all: protectedProjectProcedure
  ...
  .query(async ({ input, ctx }) => {
    ...
    const jobExecutions = await ctx.prisma.jobExecution.findMany({
      where: {
        projectId: input.projectId,  // projectId 约束应放在 findMany / findFirst 这类非主键索引的查询中
        jobOutputScoreId: {
          in: scoreIds,
        },
      },
...
async function getScoreWithEvalInfo(scoreId: string, projectId: string) {
  ...
  // id 是主键，直接用 findUnique 按 id 查询即可，不需要额外 projectId 约束
...
> **projectId 约束说明**：`projectId` 约束应放在 `findMany` / `findFirst` 这类非主键索引的查询中，主键查询直接使用 `findUnique({ where: { id } })` 即可。
...
// Prisma 代码：使用字段名 jobOutputScoreId，projectId 放在 findMany / findFirst 中
...
// id 是主键，直接用 findUnique 按 id 查询即可
...
// id 是主键，直接用 findUnique 按 id 查询即可
const [jobExecution, jobConfiguration] = await Promise.all([
  prisma.jobExecution.findUnique({ where: { id: jobExecId } }),
  prisma.jobConfiguration.findUnique({ where: { id: jobConfigId } }),
]);
...
6. **主键查询**：`id` 是主键，直接用 `findUnique({ where: { id } })` 即可，`projectId` 约束应放在 `findMany` / `findFirst` 这类非主键索引的查询中
