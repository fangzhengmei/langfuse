...
| 反查方式 | 具体操作 | 结果 |
|---------|---------|------|
| **方式 1：通过 jobOutputScoreId** | `SELECT * FROM job_executions WHERE jobOutputScoreId = 'score_primary_abc'` | ✓ 成功，找到 JobExecution je_123 |
...
| 反查方式 | 具体操作 | 结果 |
|---------|---------|------|
| **方式 1：通过 jobOutputScoreId** | `SELECT * FROM job_executions WHERE jobOutputScoreId = 'random_uuid_xyz'` | ✗ 失败，无结果（jobOutputScoreId 存储的是首条 ID） |
| **方式 2：通过 metadata map** | 从 `score.metadata['job_execution_id']` 提取 'je_123'，再查询 | ✓ 成功，找到 JobExecution je_123 |
